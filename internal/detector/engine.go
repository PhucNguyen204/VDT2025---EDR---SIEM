...
import (
	...
	"github.com/VDT2025_PhucNguyen204/internal/sigma"
...
func (e *DetectionEngine) eventToMap(event *models.Event) map[string]interface{} {
	...
	if event.Process != nil {
		...
		// Sigma compatibility for process fields
		...
		if event.Process.Name != "" {
			...
			eventMap["ProcessName"] = event.Process.Name
		}
		...
		if event.Process.Parent != nil {
			...
			// Sigma compatibility for parent process
			eventMap["ParentImage"] = event.Process.Parent.Executable
			eventMap["ParentProcessName"] = event.Process.Parent.Name
		}
	}
